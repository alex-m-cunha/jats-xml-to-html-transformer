...
(function () {
  ...
  // ---------- prevent popover content clicks from bubbling ----------
  document.addEventListener('mousedown', (ev) => {
    // Stop mousedown events inside popovers from bubbling up to triggers
    const insidePopover = ev.target.closest('.popover:not([hidden])');
    if (insidePopover && !ev.target.matches('.popover-close')) {
      ev.stopPropagation();
    }
  });
  ...
  document.addEventListener('click', (ev) => {
    // Click inside popover content (not close button) - stop bubbling to prevent trigger activation
    const insidePopover = ev.target.closest('.popover:not([hidden])');
    if (insidePopover && !ev.target.matches('.popover-close')) {
      ev.stopPropagation();
      return;
