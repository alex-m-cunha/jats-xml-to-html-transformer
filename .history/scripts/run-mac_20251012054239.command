...
ARTICLE_IMG_SUB="article-img"      # article images/figures go here, relative to JATS XML
ARTICLE_IMG_ROOT="assets/img/"     # where assets live in the output package
JAVA_HEAP="256m"
...
# Normalize: turn 15dgxd54_XML → 15dgxd54
BASE_ID="${AID%_XML}"
...
if [[ -f "$PDF_FILE" ]]; then
  ...
  echo "[info] No PDF for $BASE_ID" >> "$LOG"
fi
...
# On transform failure, write error next to the per-article log
if [[ $status -ne 0 || ! -s "$TMP_HTML" ]]; then
  tail -n 150 "$LOG" > "$ARTICLE_DIR/transformation-error.txt" || true
  osascript -e 'display alert "Transform failed" message "See transformation.log / transformation-error.txt next to the article folder"' || true
  exit 1
fi
...
[[ -f "$OUT_HTML" ]] || { osascript -e 'display alert "No HTML produced" message "Check transformation.log for details"' || true; exit 1; }
