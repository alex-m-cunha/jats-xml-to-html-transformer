...
if [[ -d "$STYLE_DIR" ]]; then
  CSS_BUNDLE="$STYLE_DIR/bundle.css"
  : > "$CSS_BUNDLE"
  ...
  find "$STYLE_DIR" -maxdepth 1 -type f -name '*.css' ! -name 'bundle.css' -delete

  JS_BUNDLE="$STYLE_DIR/bundle.js"
  : > "$JS_BUNDLE"
  while IFS= read -r -d '' js; do
    [[ "$js" == "$JS_BUNDLE" ]] && continueah
    printf '/* %s */\n' "${js##*/}" >> "$JS_BUNDLE"
    cat "$js" >> "$JS_BUNDLE"
    printf '\n\n' >> "$JS_BUNDLE"
  done < <(find "$STYLE_DIR" -maxdepth 1 -type f -name '*.js' -print0)
  find "$STYLE_DIR" -maxdepth 1 -type f -name '*.js' ! -name 'bundle.js' -delete
fi
