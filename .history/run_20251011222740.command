...
# 1) img -> images (move contents, then remove 'img')
if [[ -d "$ASSETS_DIR/img" ]]; then
  mkdir -p "$ASSETS_DIR/images"
  shopt -s dotglob nullglob
  mv "$ASSETS_DIR/img/"* "$ASSETS_DIR/images/" 2>/dev/null || true
  shopt -u dotglob nullglob
  rmdir "$ASSETS_DIR/img" 2>/dev/null || rm -rf "$ASSETS_DIR/img"
fi

# 2) css -> stylesheets (move contents)
# 3) js  -> stylesheets (move contents)
for d in css js; do
...
# 4) pdf -> multimedia (rename; if target exists, merge then remove source)
if [[ -d "$ASSETS_DIR/pdf" ]]; then
  if [[ -d "$ASSETS_DIR/multimedia" ]]; then
    rsync -a "$ASSETS_DIR/pdf/." "$ASSETS_DIR/multimedia/"
    rm -rf "$ASSETS_DIR/pdf"
  else
    mv "$ASSETS_DIR/pdf" "$ASSETS_DIR/multimedia"
  fi
fi
