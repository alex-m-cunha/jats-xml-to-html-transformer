...
<xsl:stylesheet version="1.0"
  ...
  <!-- Recursively trim trailing spaces and periods -->
  <xsl:template name="rtrim-dotspace">
    <xsl:param name="s"/>
    <xsl:choose>
      <xsl:when test="string-length($s)=0">
        <xsl:value-of select="$s"/>
      </xsl:when>
      <xsl:when test="substring($s, string-length($s))='.' or substring($s, string-length($s))=' '">
        <xsl:call-template name="rtrim-dotspace">
  ...
  <!-- Collapse any trailing spaces/dots to exactly one final '.' -->
  <xsl:template name="collapse-trailing-to-single-dot">
    <xsl:param name="s"/>
    <!-- detect if there WAS any trailing dot/space -->
    <xsl:variable name="hadTrail"
      select="string-length($s) &gt; 0 and
        (substring($s, string-length($s))='.' or substring($s, string-length($s))=' ')" />
    <!-- remove all trailing dot/space -->
    ...
    <xsl:choose>
      <xsl:when test="$hadTrail">
        <xsl:value-of select="$trimmed"/><xsl:text>.</xsl:text>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="ref">
    ...
    <li id="{$rid}" class="reference" role="doc-biblioentry">
      <span class="ref-text">
        ...
        <!-- Emit eligible nodes; ignore whitespace-only text and dot-only text following excluded links.
             Finally, if the last eligible node is text(), trim terminal '. ', ' .' or '.' only. -->
        <xsl:for-each select="(mixed-citation|element-citation)/node()[not(self::uri or self::ext-link or self::pub-id) and not(self::text() and normalize-space(.)='')]"><!-- ignore ws-only text -->
          <xsl:choose>
            <!-- Skip solitary '.' that immediately follows a removed uri/ext-link/pub-id -->
            <xsl:when test="self::text() and normalize-space(.)='.' and preceding-sibling::*[1][self::uri or self::ext-link or self::pub-id]"/>
            <!-- Last eligible text(): chop ONLY extra trailing periods -->
            <xsl:when test="position()=last() and self::text()">
              <xsl:variable name="t" select="."/>
              <!-- Will badges/links render? You already compute $_has-links below,
                   but here we recompute locally to keep this block self-contained. -->
              <xsl:variable name="_root"  select="(parent::mixed-citation | parent::element-citation)[1]"/>
              <xsl:variable name="_has-links"
                select="boolean($_root//uri | $_root//ext-link | $_root/pub-id[@pub-id-type='doi' or @pub-id-type='pmid' or @pub-id-type='pmcid'])"/>
              
              <xsl:choose>
                <!-- If badges show, strip ALL trailing spaces/dots -->
                <xsl:when test="$_has-links">
                  <xsl:call-template name="rtrim-dotspace">
                    <xsl:with-param name="s" select="$t"/>
                  </xsl:call-template>
                </xsl:when>
                
                <!-- If no badges, collapse any trailing run to a single '.' -->
                <xsl:otherwise>
                  <xsl:call-template name="collapse-trailing-to-single-dot">
                    <xsl:with-param name="s" select="$t"/>
                  </xsl:call-template>
                </xsl:otherwise>
              </xsl:choose>
      ...
      <!-- Only show badges (and leading space) if any links exist -->
      <xsl:variable name="_root"  select="(mixed-citation|element-citation)[1]"/>
      <xsl:variable name="_has-links" select="boolean($_root//uri | $_root//ext-link | $_root/pub-id[@pub-id-type='doi' or @pub-id-type='pmid' or @pub-id-type='pmcid'])"/>
      <xsl:if test="$_has-links">
        <xsl:text> </xsl:text>
            ...
            <div class="ref-text">
              ...
              <xsl:for-each select="(mixed-citation|element-citation)/node()[not(self::uri or self::ext-link or self::pub-id) and not(self::text() and normalize-space(.)='')]"><!-- ignore ws-only text -->
                <xsl:choose>
                  <!-- Skip solitary '.' that immediately follows a removed uri/ext-link/pub-id -->
                  <xsl:when test="self::text() and normalize-space(.)='.' and preceding-sibling::*[1][self::uri or self::ext-link or self::pub-id]"/>
                  <!-- Last eligible text(): trim only terminal punctuation -->
                  <xsl:when test="position()=last() and self::text()">
                    <xsl:variable name="t" select="."/>
                    <xsl:variable name="len" select="string-length($t)"/>
                    <xsl:choose>
                      <xsl:when test="$len &gt;= 2 and (substring($t, $len - 1, 2) = '. ' or substring($t, $len - 1, 2) = ' .')">
                        <xsl:value-of select="substring($t, 1, $len - 2)"/>
                      </xsl:when>
                      <xsl:when test="$len &gt;= 1 and substring($t, $len, 1) = '.'">
                        <xsl:value-of select="substring($t, 1, $len - 1)"/>
                      </xsl:when>
                      <xsl:otherwise>
